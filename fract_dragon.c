...
void	recurs_dragon(t_fract *fract, int max_iters, t_line line, double angle)
{
	...
	else
	{
		...
		recurs_dragon(fract, max_iters - 1,
			create_line(line.start.x, line.start.y, x, y), angle * fract->colors.red.sign + fract->julia.r);
		recurs_dragon(fract, max_iters - 1,
			create_line(x, y, line.end.x, line.end.y), angle * fract->colors.green.sign + fract->julia.i);
	}
}
...
void	dragon(t_fract *fract)
{
	...
	ft_bzero(fract->mlx->img_ptr, fract->mlx->width * fract->mlx->height * sizeof(int));
	start_line = create_line(fract->mlx->width / 2, 250, fract->mlx->width / 2, fract->mlx->height - 100);
	if (12 + fract->max_iters / 50 < 0)
		return ;
	recurs_dragon(fract, 12 + fract->max_iters / 50, start_line, 0.78539816339);
	// recurs_dragon(fract, fract->max_iters, start_line, 1.0472);
