...
void	pixel_put_iso(t_fract *fract, int x, int y, double z)
{
	...
	// *(int *)(fract->mlx->bonus_img_ptr +
		// (int)(((((x - y) * cos(0.523599) + fract->mlx->width / 2) + ((x + y) * sin(0.523599)
		// - (int)z) * fract->mlx->width) * fract->mlx->bonus_bpp / 8)))
		 // = mlx_get_color_value(fract->mlx->mlx_ptr, create_color_c(z, *fract));
	if (x_iso < 0 || x_iso >= fract->mlx->width || y_iso < 0 || y_iso >= fract->mlx->height)
		return ;
	*(int *)(fract->mlx->bonus_img_ptr + ((x_iso + y_iso * fract->mlx->width) *
		fract->mlx->bpp / 8)) = mlx_get_color_value(fract->mlx->mlx_ptr, create_color_c(z, *fract));
}
